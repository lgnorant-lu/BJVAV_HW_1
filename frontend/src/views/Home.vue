...
import { Send, User, Cpu, Loader2, Sun, Zap, Leaf, Sparkles, Wind, Trash2, Moon, LogOut, Hospital } from 'lucide-vue-next'
...
<template>
  <div class="flex items-center justify-center min-h-screen p-4 md:p-6 lg:p-8 transition-colors duration-500 bg-emerald-50/20">
    <!-- Main Container with Solarpunk Glassmorphism -->
    <div class="w-full max-w-5xl solar-glass rounded-[2.5rem] flex flex-col h-[92vh] overflow-hidden relative shadow-2xl border border-white/20">
      ...
      <header class="px-8 py-6 flex items-center justify-between relative z-10 border-b border-white/10 bg-white/5 backdrop-blur-xl">
        ...
        <div class="flex items-center gap-3">
          ...
          <button 
            ...
            class="flex items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-xl font-bold text-xs hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-100"
          >
            <Hospital :size="16" />
            挂号预约
          </button>
          ...
          <button 
            @click="handleLogout"
            class="p-2.5 text-emerald-600/60 hover:text-red-500 hover:bg-red-50/10 rounded-xl transition-all duration-300"
            title="退出登录"
          >
            <LogOut :size="20" />
          </button>
